/*<!--========================================
    FETCH COLLABORATION COMPANY API (DATA)
=========================================-->*/
import { api } from "../utils/api.js";

const collaborationAPI = {
    async getCollaborations() {
        const res = await api.get("/frontend/collaboration/company");
...
const collaborationUI = {
    ...
    renderCollaborations(collaborations) {
        ...
        const cards = collaborations.map((c, index) => `
            ...
            <div class="partner-item ${index >= 3 ? 'hidden' : ''} flex flex-col justify-between bg-white border-2 border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-xl hover:shadow-blue-500/50 hover:border-blue-500 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 transform">
                <div>
                    ...
                    <div class="h-24 w-full bg-slate-50 rounded-xl flex items-center justify-center p-4 mb-5">
                        <img src="/assets/media/collaborations/${c.logo}" alt="${c.organization_name}" class="max-h-full max-w-[85%] object-contain transition-all duration-300">
                    </div>
                    <!-- Partner Info -->
                    <h3 class="text-xl font-bold text-slate-800 mb-2 hover:text-blue-800 transition-colors text-center">${c.organization_name}</h3>
                ...
                <div class="mt-6 pt-4 border-t border-gray-50 flex justify-between items-center">
                    <span class="text-xs font-medium text-amber-600 bg-amber-50 px-2.5 py-1 rounded-full">${c.collaboration_type}</span>
