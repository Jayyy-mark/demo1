...
export const laboratoryUI = {
    /**
     * Group laboratories by laboratory_name, render ONE row per unique name.
     */
    render(laboratories) {
        tbody.empty();
        ...
        Object.entries(groups).forEach(([name, records]) => {
            ...
            // Thumbnail strip (up to 3, +N badge for more)
            ...
            <tr>
                ...
                <td class="py-3 text-start">${first.category || "N/A"}</td>
                <td class="py-3 text-start">${first.description || "N/A"}</td>
     ...
     * @param {Array} records - all laboratory rows for this group
     */
    fillUpdateForm(records) {
        const first = records[0];
        const form  = $("#dataForm-update");

        form.find('[name="original_name"]').val(first.laboratory_name);
        form.find('[name="laboratory_name"]').val(first.laboratory_name);
        ...
        withImages.forEach(record => {
            ...
            item.innerHTML = `
                ...
                <button type="button" class="img-preview-remove lab-update-img-remove" title="Mark for deletion">
                    <i class="ti ti-trash"></i>
                </button>
                <div class="img-delete-overlay" style="display:none;">
                    ...
                    <button type="button" class="img-undo-btn lab-update-img-cancel" title="Undo deletion mark">
